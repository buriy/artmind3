...
public class Columns {
		...
		for (SegmentUpdate segmentUpdate : segmentUpdateList[bit][cell]) {
			...
			if (positiveReinforcement) {
				for (Integer u : updates) {
					if (synapses.containsKey(u)) { // synapses & updates
						addPermanence(u, targetSegment);
					} else { // synapses & !updates
						setPermanence(u, targetSegment);
					}
				}
				ArrayList<Integer> removes = new ArrayList<Integer>();
				for (Integer s : synapses.keySet()) { // !synapses & updates
					if (!updates.contains(s)) {
						removes.add(s);
					}
				}
				for (Integer s : removes) {
					decPermanence(s, targetSegment);
				}
			} else {
				for (Integer u : updates) {
					if (synapses.containsKey(u)) {
						decPermanence(u, targetSegment);
					}
				}
			}
		}
	...
	private void decPermanence(int key, Segment s) {
		...
		if (perm <= 0 && opt.REMOVE_ZERO_CONNECTIONS) {
			permanences[key].remove(s);
			s.synapses.remove(key);
		}
	}

	// private void decPermanence(HashMap<Integer, Integer> synapses, HashSet<Integer>
	// connected,
	// int key) {
	// Integer perm = synapses.get(key);
	// if (perm == null)
	...
	/**
	 * This routine returns true if the number of synapses on segment s that are active
	 * due to the given state at time t is greater than activationThreshold. The parameter
	 * state can be activeState, or learnState.
	 */
